import { BlogError } from "../../runtime/blog-error.js";
import { EngagementRepository } from "./engagement.repository.js";
import type { EngagementInput, FavoriteInput } from "./engagement.types.js";
export class EngagementService {
  constructor(private readonly repository = new EngagementRepository()) {}
  summary(id: number) {
    return this.repository.summary(id);
  }
  favorites(actorKey: string) {
    return this.repository.favorites(actorKey.trim());
  }
  async setFavorite(input: FavoriteInput) {
    if (!(await this.repository.articlePublished(input.articleId)))
      throw BlogError.notFound("Published article was not found.");
    return this.repository.setFavorite({
      ...input,
      actorKey: input.actorKey.trim(),
    });
  }
  async upsert(input: EngagementInput) {
    if (!(await this.repository.articlePublished(input.articleId)))
      throw BlogError.notFound("Published article was not found.");
    if (input.kind === "star" && (!input.rating || input.rating < 1 || input.rating > 5))
      throw BlogError.validation("A star rating from 1 to 5 is required.");
    return this.repository.upsert({
      ...input,
      actorKey: input.actorKey.trim(),
      rating: input.kind === "star" ? input.rating : null
    });
  }
}
